...
func (s ss) Method11(i int) string  { return "结构体方法1" }
func (s *ss) Method22(i int) string { return "结构体方法2" }
...
var complexTypes2 = []interface{}{
	...
	structValue.Method11, structValue.Method22, // 方法
...
func main() {
	// 测试简单类型
	//for i := 0; i < len(simpleTypes2); i++ {
		PrintInfo2(simpleTypes2[0])
	//}
	// 测试复杂类型
	//for i := 0; i < len(complexTypes2); i++ {
	//	PrintInfo2(complexTypes2[i])
	//}
	// 测试单个对象
	//PrintInfo2(&unsafeP)
	//PrintInfo2(nilInterfece)
	// PrintInfo2(&nilInterfece) // 会引发 panic
}

func PrintInfo2(i interface{}) {
	if i == nil {
		fmt.Println("--------------------")
		fmt.Printf("无效接口值：%v\n", i)
		fmt.Println("--------------------")
